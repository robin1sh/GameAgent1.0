"""
统一环境入口：make_mario_env、make_jumper_env、make_coinrun_env、make_vec_env。
"""
from .mario_env import make_mario_env
from .jumper_env import make_jumper_vec_env, PROCGEN_AVAILABLE
from .coinrun_env import make_coinrun_vec_env
...
def make_unified_env(env_name: str):
    ...
    if env_name.lower() == "jumper":
        ...
        return venv
    if env_name.lower() == "coinrun":
        if not PROCGEN_AVAILABLE:
            raise ImportError("procgen 未安装，无法使用 coinrun。请运行: pip install procgen")
        venv = make_coinrun_vec_env(n_envs=1)
        return venv
...
def make_vec_env(
    ...
    env_name: "mario" | "jumper" | "coinrun" | "both"
    """
    if env_name.lower() == "mario":
...
def make_coinrun_env(n_envs=10):
    """兼容接口：创建 coinrun VecEnv。"""
    return make_coinrun_vec_env(n_envs=n_envs)
