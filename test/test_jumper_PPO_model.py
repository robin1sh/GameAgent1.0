"""
专门用于测试 Jumper 的 PPO 模型（Stable-Baselines3 .zip）。
...
def parse_args():
    parser = argparse.ArgumentParser(description="测试 Jumper PPO 模型并可视化")
    parser.add_argument("--model", default="models/ppo_jumper.zip", help="SB3 模型路径")
...
def main():
    ...
    if args.render:
        ...
        pygame.display.set_caption("Jumper PPO 预览")
    ...
    for _ in range(args.steps):
        ...
        if args.render and screen is not None:
            # ProcgenObsWrapper 会把彩色帧放到 infos[0]["rgb"]，优先显示彩色画面
