...
| 项目 | 要求 | 说明 |
|------|------|------|
| 观测 | 64×64 灰度，4 帧堆叠 | 与 `VecFrameStack` + `channels_order="last"` 一致 |
...
| 1.5 | 模仿学习训练脚本（可分游戏或混合） | `train/train_cnn_imitation_unified.py` | 已实现（当前 `both`=人类 mario+jumper） |
| 1.6 | 保存 backbone 供阶段 2 PPO 加载 | 输出 `mario_backbone.pt`、`jumper_backbone.pt` 或统一 `imitation_backbone.pt` | 已实现 |

### 1.5 关键实现要点

**record_mario_unified.py**：
- 使用 `make_mario_env()`（COMPLEX_MOVEMENT + 64×64 灰度 + SkipFrame）
...
- 扫描 `data_imitation_unified/` 下各子目录（mario、jumper、rl_expert_mario、rl_expert_jumper）
- 每样本：4 张 PNG 堆叠为 (64, 64, 4)，`label.txt` → 0–14
- 支持 `include_sources=["mario", "jumper", "rl_expert_mario", "rl_expert_jumper"]` 过滤

**train_cnn_imitation_unified.py**：
- CNN backbone 与 `CustomCNN` 结构一致（Conv + Linear → 512 维）
- 分类头：`Linear(512, 15)`
- 可分游戏训练输出 `mario_backbone.pt`、`jumper_backbone.pt`，或混合训练输出统一 backbone

### 1.6 验证检查点

- [ ] `UnifiedImitationDataset` 能正确加载，`len(dataset) > 0`
- [ ] `obs.shape == (64, 64, 4)`，`label in range(15)`
...
| 2.3 | 确保 CustomCNN 与 backbone 结构一致 | `models/custom_cnn.py` | 已就绪 |
